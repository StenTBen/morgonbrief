...
async function main() {
  ...
  if (A.enabled) {
    ...
    const mineEpisodes = allMine
      ...
      .map((e) => ({
        sphere: 'val2026',
        // One pill per episode, so the label has to distinguish them. Time plus
        // strength turns the player into a readable timeline of the day.
        ...
        itemCount: 1,
        // Empty on purpose: deepSection() only renders episodes with items, and
        // these entries are already in the feed above.
        items: []
      }));

    const others = (feed.episodes ?? []).filter((e) => e.sphere !== 'val2026');
    feed.episodes = [...mineEpisodes, ...others];
    console.log(`  episodes: ${mineEpisodes.length} val2026 (newest first), ${others.length} from the nightly run`);
  }
