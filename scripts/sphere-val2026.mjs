...
import { req, makeDeepseek, sweepMedia, fetchArticleText, scriptSystemPrompt } from './lib.mjs';
...
async function researchAndWrite(candidate, sphere, config, { deepseek, L }) {
  ...
  return {
    ...
    sphere: 'val2026',
    kategori: (out.kategori || 'VAL').toUpperCase().slice(0, 24),
...
async function main() {
  ...
  if (now < new Date(sphere.pulse.from) || now > new Date(sphere.pulse.until)) {
  ...
  for (const candidate of chosen) {
    try {
      entries.push(await researchAndWrite(candidate, sphere, config, { deepseek, L }));
      ledger.published.push({ key: candidate.key, strength: candidate.strength, at: nowIso });
    } catch (e) {
  ...
  // The nightly run rewrites docs/feed.json wholesale at 02:00 and will drop
  // these. By then the window has closed and val2026 is an ordinary nightly
  // sphere, so that is the intended handover rather than a loss.
  ...
  feed.feed = [...entries, ...(feed.feed ?? []).filter((e) => !mine.has(e.id))];
  feed.builtAt = nowIso;
...
// Unlike run.mjs this does not exit non-zero on an empty result. Only a crash
// should surface in Actions.
