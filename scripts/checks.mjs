/**
 * Offline checks on the things that fail silently.
 *
 * No network, no model calls, no API keys. Runs in well under a second and sits
 * in front of the nightly build in brief.yml, so a broken wiring change is
 * caught before any money is spent on it.
 *
 * WHAT BELONGS HERE. Not coverage. Every case is something that has actually
 * gone wrong in this pipeline, or would have cost money or quality to discover
 * any other way - and the common thread is that all of them SUCCEED while being
 * wrong:
 *   - a price table keyed to a model nobody calls, so every run reported
 *     UNKNOWN cost and the daily ceiling had no instrument behind it
 *   - a json prompt DeepSeek rejects, which published nothing for two days
 *     while the log read like a quiet news cycle
 *   - a wedding bombing selected as Sveriges politik
 *   - "C stänger dörren till M" searched as "stänger dörren", both parties
 *     dropped below a length floor before the query ran
 *   - a model role routed to a model with no rate behind it
 *   - implementation detail in profile.json rendered into a prompt
 * A test that only fails when something is already obviously broken is not
 * worth the second it costs.
 *
 * IT BLOCKS THE BUILD. That is deliberate and has a cost worth knowing: the
 * night sphere ids or roles change, this will fail and there will be no brief
 * until it is updated. The checks are pure and deterministic - no clock, no
 * network - so a failure is always a real inconsistency and never flakiness.
 * The failure output names the case and reprints whatever the code under test
...
/**
 * The harness. Two things it does that the obvious version does not.
 *
 * IT AWAITS. The first version called fn() and caught synchronously, so an
 * async test that failed produced an unhandled rejection instead of a FAIL -
 * the suite would report green while a case had actually blown up. Cases are
 * queued and run in order at the end, each one awaited.
 *
 * IT SWALLOWS THE MODULES' OWN LOGGING, and reprints it only under a failure.
 * The code under test narrates itself: missing keys, fallbacks, selection
 * decisions. In a passing run that output is the test's own fixtures talking,
 * and in a workflow log it is indistinguishable from the real thing going
 * wrong - a green step that prints "claude-opus-5 failed" and "BRAVE_API_KEY is
 * unset" teaches you to skim past exactly the lines that matter on the night
 * they are true.
 */
const queue = [];
const notes = [];
...
async function runAll() {
  ...
  for (const [name, fn] of queue) {
    ...
    try {
      ...
      for (const l of captured) results.push(`      | ${l}`);
    }
  }
  console.log = realLog;
  return results;
...
t('a realistic night stays far inside the dollar', () => {
  ...
  note(`estimated DeepSeek spend for a 9-call night at current rates: $${total.toFixed(4)}`);
});
...
t('party abbreviations survive both query builders', () => {
  // From the live run: "C stänger dörren till M" produced the query
  // "stänger dörren" - both parties, the whole subject, dropped below the
  // length floor before the search ran.
  const c = { items: [{ title: 'C stänger dörren till M' }] };
  for (const [kind, q] of [['open', queryFor(c)], ['topic', topicQueryFor(c)]]) {
    assert.ok(/\bC\b/.test(q), `${kind} query lost C: "${q}"`);
    assert.ok(/\bM\b/.test(q), `${kind} query lost M: "${q}"`);
  }
  assert.ok(topicQueryFor(c).startsWith('C M'), 'acronyms should lead the restricted query');
});
t('multi-letter party and institution acronyms survive too', () => {
  const c = { items: [{ title: 'SD och KD kräver besked av M om EU-budgeten' }] };
  const q = topicQueryFor(c);
  for (const a of ['SD', 'KD', 'M']) assert.ok(new RegExp(`\\b${a}\\b`).test(q), `lost ${a}: "${q}"`);
});
t('a sentence-opening capital is still not treated as an acronym', () => {
  const q = topicQueryFor({ items: [{ title: 'Regeringen backar om utredningen efter kritik från Lagrådet' }] });
  assert.ok(/Lagrådet/.test(q), `lost the institution: "${q}"`);
  assert.ok(!q.startsWith('Regeringen'), `"Regeringen" led the query: "${q}"`);
});
...
if (failed.length) {
  ...
  // Nothing but the count on a green run. Anything the modules printed was
  // fixture noise; anything worth reading is in the build step below this one.
  for (const n of notes) console.log(`note: ${n}`);
}
console.log(`${queue.length - failed.length}/${queue.length} checks passed`);
