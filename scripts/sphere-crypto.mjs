...
export async function linkCoverage(items, { deepseek, today, governanceIndex, sphere, limits = {} }) {
  ...
  const forumDigest = governanceIndex
    .slice(0, limits.maxGovernanceTopics ?? 400)
  ...
  const out = await deepseek(
    ...
    { json: true, maxTokens: limits.tokensLinking ?? 16000, role: 'crypto-linking' }
  );
...
export async function writeCards(docs, { deepseek, limits = {} }) {
  ...
  return deepseek(
    ...
    { json: true, maxTokens: limits.tokensCards ?? 16000, role: 'crypto-cards' }
  ...
  return deepseek([
    { role: 'system', content: scriptSystemPrompt(sphere, config) },
    {
      ...
      content:
        ...
        'Within SUBSTANCE, go past what the headlines already said and into the primary source - the forum ' +
        'thread or the chain\'s own numbers. Close the episode by saying plainly which claims rest on a ' +
        'single source. Plain text only.'
    }
  ], { maxTokens: config.limits?.tokensScript ?? 16000, role: 'crypto-script' });
}
