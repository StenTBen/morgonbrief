...
/**
 * Which knowledgeLevel key each sphere reads.
 *
 * A sphere and a knowledge domain are not the same thing - Världen covers both
 * international politics and congressional procedure, and he knows one far
 * better than the other. Where a sphere spans two domains, the taps move the
 * one the sphere is primarily about, and the other stays where it is.
 */
