...
import { makeAnthropic, makeRouter } from './model.mjs';
...
const deepseek = makeDeepseek(DEEPSEEK_KEY, {
  ...
  onUsage: (usage, role, model) => recordCall(run, { role, model: model ?? DEEPSEEK_MODEL, usage })
});
...
/**
 * One callable for the whole pipeline, routing by role.
 *
 * Named `write` rather than `deepseek` at this level because it is no longer
 * one provider. The sphere modules still receive it under the parameter name
 * `deepseek` and still call it the same way - the routing is invisible to them
 * on purpose, so that changing which model writes an episode is a config edit
 * and never a code change.
 */
const anthropic = makeAnthropic(process.env.ANTHROPIC_API_KEY ?? '', {
  onUsage: (usage, role, model) => recordCall(run, { role, model, usage })
});
const write = makeRouter(config, { deepseek, anthropic });

// --------------------------------------------------------------- pod tier

async function podCongress(sphere, coverage, seen) {
  const candidates = await Congress.linkCoverage(coverage, { deepseek: write, today, limits: config.limits });
  const scored = [];
  ...
  const [cards, script] = await Promise.all([
    Congress.writeCards(chosen, { deepseek: write, limits: config.limits }),
    Congress.writeScript(chosen, sphere, { deepseek: write, today, config })
  ]);
...
async function podCrypto(sphere, coverage, seen) {
  ...
  const candidates = await Crypto.linkCoverage(coverage, { deepseek: write, today, governanceIndex, sphere, limits: config.limits });
  const scored = [];
  for (const c of candidates) {
    try {
  ...
  const chosen = pick(scored, sphere);
  const [cards, script] = await Promise.all([
    Crypto.writeCards(chosen, { deepseek: write, limits: config.limits }),
    Crypto.writeScript(chosen, sphere, { deepseek: write, today, config })
  ]);
...
async function podWorld(sphere, coverage, seen) {
  ...
  const clusters = await World.findGaps(coverage, swedish, { deepseek: write, today, sphere, limits: config.limits });
  ...
  const [cards, script] = await Promise.all([
    World.writeCards(chosen, { deepseek: write, limits: config.limits }),
    World.writeScript(chosen, sphere, { deepseek: write, today, config })
  ]);
...
async function politicalFallback(sphere) {
  ...
  const script = await write(
    ...
    { maxTokens: config.limits?.tokensScript ?? 16000, role: 'congress-fallback-script' }
  );
...
async function podSverige(sphere, coverage, seen) {
  return Sverige.runEpisode(sphere, coverage, seen, { deepseek: write, config, research });
}
...
async function main() {
  ...
  // AND ONLY NOW is profile.json read - this is the "anything" the comment
  // above is about.
  //
  // applyFeedback has just rewritten these numbers. Reading them at module load
  // beside config.json would be reading yesterday's copy, so every level change
  // would take effect a day after the log announced it. That is the failure the
  // ordering exists to prevent, and putting this read anywhere earlier
  // reintroduces it.
  //
  // Merged into `config` rather than threaded through as a new argument,
  // because `config` is already handed to every sphere's writeScript and on to
  // scriptSystemPrompt. One assignment reaches all four spheres and no sphere
  // module changes.
  //
  // Never fatal: without it the prompts fall back to the static prose in
  // config.json, which is exactly how they read before the loop was wired up.
  try {
    ...
    config.listener = { ...(config.listener ?? {}), levels };
    const named = Object.entries(levels)
      .filter(([k, v]) => !k.startsWith('_') && Number.isFinite(Number(v?.level)))
      .map(([k, v]) => `${k} ${v.level}`)
      .join(', ');
    console.log(`Levels: ${named || 'none readable'}`);
  } catch (e) {
    console.log(`Levels: profile.json unreadable (${e.message}) - prompts fall back to the prose in config.json`);
  }
  ...
  let feed = [];
  try {
    feed = await buildFeed(allCoverage, { deepseek: write, today, limits: config.limits });
  } catch (e) {
