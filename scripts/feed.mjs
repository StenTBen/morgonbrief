...
async function writeEntries(clusters, { deepseek }) {
    ...
    [
      ...
      },
      {
        ...
        content:
          ...
          'Return {"entries":[{"clusterIndex":0,"rubrik":"...","sammanfattning":"3-5 sentences covering what ' +
          'happened and the substance behind it","synthes":"one sentence on what the sources add to or ' +
          'contradict in each other, or empty string when they simply agree"}]}\n\n' +
  ...
  return entries
    .map((e) => {
      ...
      return {
        id: `feed-${cluster.items[0].sphere}-${Buffer.from(cluster.label || cluster.items[0].title).toString('base64url').slice(0, 24)}`,
        sphere: cluster.items[0].sphere,
        rubrik: e.rubrik ?? cluster.items[0].title,
